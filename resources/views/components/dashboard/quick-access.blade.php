...
<div class="grid {{ $cols }} gap-4 lg:gap-5">
    @foreach ($modules as $module)
        @php
            $title = $module['title'] ?? '';
            $icon = $module['icon'] ?? 'fa-solid fa-cube';
        ...
        @if (!$enabled)
            <div class="bg-bg-primary border border-border-light rounded-2xl p-6 opacity-50 cursor-not-allowed relative overflow-hidden">
                ...
                <span
                    class="relative z-10 inline-flex items-center gap-1.5 bg-yellow-50 text-yellow-700 px-2.5 py-1 rounded-lg text-xs font-medium leading-normal">
        ...
        @else
            <a href="{{ $url }}"
               class="bg-bg-primary border border-border-light rounded-2xl p-6 hover:border-{{ $color }}-300 hover:shadow-md hover:-translate-y-1 transition-all duration-200 group relative overflow-hidden">
                <div
                    class="absolute inset-0 bg-gradient-to-br {{ $gradientColor }} via-white/40 to-transparent opacity-70 group-hover:opacity-90 transition-opacity duration-200"></div>
                <div
                    class="relative z-10 w-12 h-12 {{ $bgColor }} rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-200">
                    <i class="{{ $icon }} text-xl {{ $iconColor }}"></i>
                </div>
