            ...
            <div>
                ...
                <div class="space-y-2">
                    @foreach($documents as $doc)
                        ...
                        @endphp
                        <div data-modal-open="fileModal" data-modal-data='@json($doc)' class="flex items-center gap-3 bg-white rounded-2xl p-3 shadow-sm hover:shadow-lg transition-all cursor-pointer active:scale-[0.98]">
                            <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 {{ $typeStyle['bg'] }} {{ $typeStyle['color'] }}">
                                <i class="fa-solid {{ $typeStyle['icon'] }}"></i>
                            </div>
                            <div class="flex-1 min-w-0">
                                <h3 class="text-slate-900 text-sm font-medium truncate">{{ $doc['title'] ?? $doc['name'] ?? 'بدون عنوان' }}</h3>
                                <p class="text-slate-500 text-xs">{{ $doc['created_at']['human']['default'] ?? '—' }}</p>
                            </div>
