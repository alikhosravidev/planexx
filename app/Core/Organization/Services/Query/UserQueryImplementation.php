...
use App\Domains\User\ActiveUserStatsDTO;
...
readonly class UserQueryImplementation implements UserQuery
{
    ...
    {
        $employee = UserTypeEnum::Employee->value;
        $customer = UserTypeEnum::Customer->value;
        $user     = UserTypeEnum::User->value;

        $row = $this->repository
            ->newQuery()
            ->selectRaw('SUM(CASE WHEN is_active = 1 THEN 1 ELSE 0 END) as total_active')
            ->selectRaw("SUM(CASE WHEN is_active = 1 AND user_type = {$employee} THEN 1 ELSE 0 END) as employees_active")
            ->selectRaw("SUM(CASE WHEN is_active = 1 AND user_type = {$customer} THEN 1 ELSE 0 END) as customers_active")
            ->selectRaw("SUM(CASE WHEN is_active = 1 AND user_type = {$user} THEN 1 ELSE 0 END) as users_active")
            ->first();

        return new ActiveUserStatsDTO(
            (int) ($row->total_active ?? 0),
            (int) ($row->employees_active ?? 0),
            (int) ($row->customers_active ?? 0),
            (int) ($row->users_active ?? 0),
        );
    }
