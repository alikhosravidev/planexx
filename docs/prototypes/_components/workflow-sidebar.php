<?php
/**
 * کامپوننت Sidebar ماژول مدیریت وظایف
 */
...
<aside class="hidden lg:flex flex-col w-[280px] bg-bg-primary border-l border-border-light min-h-screen sticky top-0 self-start">
  ...
  <div class="px-6 py-6 border-b border-border-light">
    <a href="/dashboard/index.php" class="flex items-center gap-3 hover:opacity-80 transition-opacity">
      <div class="w-10 h-10 bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-lg flex items-center justify-center flex-shrink-0">
  ...
  <!-- Module Title -->
  <div class="px-6 py-4 bg-indigo-50 border-b border-border-light">
    <div class="flex items-center gap-2.5">
      <i class="fa-solid fa-list-check text-indigo-600 text-xl flex-shrink-0"></i>
      <h2 class="text-base font-bold text-text-primary leading-normal">مدیریت وظایف</h2>
    </div>
  </div>
  ...
  <nav class="flex-1 overflow-y-auto px-3 py-4">
    <div class="space-y-1">
      <?php foreach ($workflowMenuItems as $item): ?>
        <?php 
        $isActive = str_contains($_SERVER['PHP_SELF'], str_replace('/dashboard/workflows/', '', $item['url']));
        $baseClasses = "flex items-center gap-3 px-4 py-3 rounded-lg text-base font-medium transition-all duration-200";
        
        if ($isActive) {
            $classes = $baseClasses . " bg-indigo-600 text-white shadow-sm";
        } else {
...
<div class="lg:hidden fixed bottom-6 left-6 z-40">
  <button 
    ...
    class="w-14 h-14 bg-indigo-600 text-white rounded-full shadow-lg flex items-center justify-center hover:bg-indigo-700 transition-all duration-200">
...
<div 
  ...
  <div 
    ...
    <div class="px-6 py-4 bg-indigo-50 border-b border-border-light">
      <div class="flex items-center gap-2.5">
        <i class="fa-solid fa-list-check text-indigo-600 text-xl flex-shrink-0"></i>
        <h2 class="text-sm font-bold text-text-primary leading-normal">مدیریت وظایف</h2>
      </div>
    </div>
